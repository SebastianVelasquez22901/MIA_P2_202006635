...
func mkfs(id string, t string, fs string) {
	...
	if Comparar(fs, "2fs") {
		...
		fechaAntigua := time.Date(1900, 1, 1, 0, 0, 0, 0, time.UTC).String()
		copy(spr.S_umtime[:], fechaAntigua)
		spr.S_mnt_count = spr.S_mnt_count + 1
		spr.S_filesystem_type = 2
		ext2(spr, particion, int64(n), p)
	} else if Comparar(fs, "3fs") {

		/*
			n = ((particion.size-sizeOf(superblock) /sizeOf(Journaling)+sizeOf(inodos) + 3* sizeOf(block) + 4)

		*/

		p := ""
		particion := GetMount("MKFS", id, &p)
		n := math.Floor(float64(particion.Part_size-int64(unsafe.Sizeof(Structs.SuperBloque{})))/float64(int64(unsafe.Sizeof(Structs.Journaling{}))) + float64(unsafe.Sizeof(Structs.Inodos{})+3*unsafe.Sizeof(Structs.BloquesArchivos{})+4))
		spr := Structs.NewSuperBloque()
		spr.S_magic = 0xEF53
		spr.S_inode_size = int64(unsafe.Sizeof(Structs.Inodos{}))
		spr.S_block_size = int64(unsafe.Sizeof(Structs.BloquesCarpetas{}))
		...
		Error("MKFS", "El sistema de archivos no es válido")
...
func ext2(spr Structs.SuperBloque, p Structs.Particion, n int64, path string) {
	...
	spr.S_firts_ino = 1
	spr.S_first_blo = 1
	MitadBA := (p.Part_size - spr.S_block_start) / 2
	MitadBA = MitadBA + spr.S_block_start
	...
	for i := 0; i < int(n); i++ {
		EscribirBytes(file, []byte{byte(zero)})
	}
	...
	//Aqui recupero el superbloque para saber si esta bien
	...
	if err_ != nil {
		Error("MKFS", "Error al leer el archivo")
		return
	}
	file.Close()
	...
	EscribirBytes(file, []byte{byte(caracter)})
	EscribirBytes(file, []byte{byte(caracter)})
...
func ext3(spr Structs.SuperBloque, p Structs.Particion, n int64, path string) {
	spr.S_bm_inode_start = p.Part_size + int64(unsafe.Sizeof(Structs.SuperBloque{})) + (int64(unsafe.Sizeof(Structs.Journaling{})) * n)
	spr.S_bm_block_start = spr.S_bm_inode_start + n
	spr.S_inode_start = spr.S_bm_block_start + (3 * n)
	spr.S_block_start = spr.S_bm_inode_start + (n * int64(unsafe.Sizeof(Structs.Inodos{})))
	spr.S_firts_ino = 1
	...
	InicioJournal := p.Part_start + int64(unsafe.Sizeof(Structs.SuperBloque{}))

	file.Seek(InicioJournal, 0)
	SizeJournal := int64(unsafe.Sizeof(Structs.Journaling{}))
	tempJ := Structs.NewJournal()
	tempJ.Journaling_start = InicioJournal
	copy(tempJ.Operacion[:], "mkdir")
	copy(tempJ.Path[:], "/")
	copy(tempJ.Contenido[:], "---")
	copy(tempJ.Fecha[:], fecha)
	var bin7 bytes.Buffer
	binary.Write(&bin7, binary.BigEndian, tempJ)
	EscribirBytes(file, bin7.Bytes())
	InicioJournal += SizeJournal

	file.Seek(InicioJournal, 0)
	tempJ.Journaling_start = InicioJournal
	copy(tempJ.Operacion[:], "mkfile")
	copy(tempJ.Path[:], "/users.txt")
	copy(tempJ.Contenido[:], dataArchivo)
	copy(tempJ.Fecha[:], fecha)
	var bin8 bytes.Buffer
	binary.Write(&bin8, binary.BigEndian, tempJ)
	EscribirBytes(file, bin8.Bytes())
	...
	Mensaje("MKFS", "Se ha formateado la partición "+nombreParticion+" correctamente con ext3")
}
